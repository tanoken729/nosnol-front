    ...
    <div class="user-music-file-detail">
      ...
      <div class="user-music-file-detail-header">
        ...
        <div class="user-info">
          ...
          <div v-for="(userDetailItem, index) in userDetailItems" :key="index">
            ...
            <img
              :src="
                `${$axios.defaults.baseURL}storage/${userDetailItem[0].user_icon}`
              "
              alt=""
              class="user-icon"
            />
          ...
          <div class="user-status">
            <!-- クリエイターネーム -->
            <h2 v-for="(userDetailItem, index) in userDetailItems" :key="index">
              {{ userDetailItem[0].user_name }}
            </h2>
            <!-- 自己紹介 -->
            <p
              class="click-file-user-description"
              v-for="(userDetailItem, index) in userDetailItems"
              :key="index"
            >
              {{ userDetailItem[0].description }}
            </p>
            <!-- フォロー数フォロワー数 -->
      ...
      <div class="full-page">
        ...
        <div class="content-fit">
          <div
            class="content"
            v-for="(userDetailItem, index) in userDetailItems.userDetailItems"
            :key="index"
            @click="
              setMusicFileData(
                userDetailItem.title,
                userDetailItem.cover_image,
                userDetailItem.music_file,
                userDetailItem.user_name,
                userDetailItem.user_id,
                userDetailItem.id,
                userDetailItem.description,
                userDetailItem.user_icon
              )
            "
          >
            <nuxt-link
              :to="{
                name: 'user-title',
                params: {
                  user: `${userDetailItem.user_name}`,
                  title: `${userDetailItem.title}`
                }
              }"
            >
              ...
              <div v-if="userDetailItem.title">
                <img
                  :src="
                    `https://nosnol-production-image-and-audio.s3.ap-northeast-1.amazonaws.com/${userDetailItem.cover_image}`
                  "
                  class="cover-image"
                />
                <h3 class="userDetailItem-title">{{ userDetailItem.title }}</h3>
                <NuxtLink to="/userdetail"
                  ><h3 class="userDetailItem-user-name">
                    {{ userDetailItem.user_name }}
                  </h3></NuxtLink
                >
                <audio v-bind:id="`bgm-${index}`" preload>
                  <source
                    :src="
                      `https://nosnol-production-image-and-audio.s3.ap-northeast-1.amazonaws.com/${userDetailItem.music_file}`
                    "
                    type="audio/mp3"
                  />
                </audio>
            ...
            <div v-if="userDetailItem.title">
              <font-awesome-icon
                ...
                class="ellipsis-h"
                @click="deleteMusicFile(userDetailItem.id)"
              />
              <button
                v-if="play === index"
