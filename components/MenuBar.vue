    ...
    <div id="main-content">
      <ul>
        ...
        <nuxt-link
          :to="{
            name: 'user',
            params: { user: `${$store.state.auth.user.name}` }
          }"
          ><li
            id="user-name"
            @click="setMusicFileData($store.state.auth.user.id)"
          >
            {{ $store.state.auth.user.name }}
          </li></nuxt-link
        >
        <NuxtLink to="/profilesettings"
          ><li>
            <font-awesome-icon
              :icon="['fas', 'cogs']"
              class="cogs-font-solid"
            />設定
          </li></NuxtLink
        >
        <li @click="logout">
